...
test_loss = model.evaluate(x_train, x_train, batch_size=128)
print("Test loss:", test_loss)
preds = model.predict(x_train)

mse = np.mean(np.square(x_train - preds), axis=(1, 2, 3))
bce = np.mean(-(x_train * np.log(preds + K.epsilon()) + (1 - x_train) * np.log(1 - preds + K.epsilon())), axis=(1, 2, 3))
Normal_score = mse + bce
sns.histplot(Normal_score, kde=True)
plt.xlabel("Normal_score")
plt.show()
...
for i, ax in enumerate(axes.flat):
    if i < len(anomaly_images):
        ...
        ax.set_title(f"Normal_score: {anomaly_score[i]:.2f}")
    ax.axis('off')
plt.tight_layout()
plt.show()